...
prox_null_model <-
glmmTMB(prox_two_month_perm ~ male_hybridscore + female_hybridscore + females_in_grp_avg + males_in_grp_avg + female_age_avg + female_rank_avg + male_rank_avg + female_rank_avg*male_rank_avg + female_age_transform_with_avg + reproductive_state_binary2 + assortative_max + male_hybridscore*reproductive_state_binary2 + female_hybridscore*reproductive_state_binary2 + observer_effort_two_months + gen_diversity_male + gen_diversity_female + QG_final + (1 | female_sname) + (1 | male_sname), data = prox_null_perm, family = "binomial")
...
write.table(permuted_results,file=paste0("GLMMTMB_permuted_results_prox2_noC_repstate_recoded",seed,".txt"),quote=F,col.names=F,row.names=F)
q(save="no")
