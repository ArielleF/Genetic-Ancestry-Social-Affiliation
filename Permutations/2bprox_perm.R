...
permuted_results=Reduce(rbind,parLapply(clus,1:nperm,function(perm){
  ...
  prox_null_unique_prob$prox_prob_perm <- sample(prox_null_unique_prob$prox_prob,2338,replace=F) # randomly permute proximity probabilities across all female two-month intervals; note that 2338 is the number of unique female two-month intervals in the proximity data set
  ...
  print(perm)
  return(unlist(fixef(prox_null_model))[1:19]) # output intercept and all predictor variables in the model
}))


write.table(permuted_results,file=paste0("prox_permuted_results_",as.numeric(Sys.getenv(c("SLURM_ARRAY_TASK_ID"))),".txt"),quote=F,col.names=F,row.names=F) # output results as text file with name that identifies the job number within the array (i.e., 1-50) so that this file doesn't overwrite other job outputs from the array
